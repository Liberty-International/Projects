...
;function GetPageName(){var a=decodeURIComponent(window.location.pathname);pageName=a.substring(a.lastIndexOf("/")+1)}function SetCookies(){pageName==""&&GetPageName();var a=0,b=0,c;$("input:checkbox, input:radio, input:text, input:hidden, select, textarea").each(function(){var c=$(this);elementId=c.attr("id");if(b>=150)return!1;if(c.val().trim().length>0&&(c.is("input:text, textarea")&&elementId.indexOf("aux")<1&&c.attr("readonly")!="readonly"||c.is("select"))){elementVal=c.val().trim();if(elementVal.length>3701)return!0;$.cookie(pageName+"_"+elementId,elementVal),a+=elementVal.length}else c.is("input:checkbox, input:radio")&&(elementVal=c.attr("checked"),elementVal!=undefined&&($.cookie(pageName+"_"+elementId,!0),a+=elementVal.length));b++}),recalc_onclick("");var d=document.location.search;d.length>0&&d.toUpperCase().search("&FORCE=TRUE")>-1&&LoadFromQueryString()}function GetCookies(){pageName==""&&GetPageName();if(document.cookie&&document.cookie!=""){var a,b,c,d,e=document.cookie.split(";");for(var f=0;f<e.length;f++){a=e[f].split("=");var g=decodeURIComponent(a[0]).trim();c=g.substring(pageName.length+1,g.length),b=$("#"+c),b.is("input:text, input:hidden, textarea")||b.is("select")?(b.val($.cookie(g)),b.css("display")=="none"&&b.attr("data-widgettype")!=undefined&&SetWidgetValue(b)):$.browser.msie!=undefined?b.attr("checked",!0):document.getElementById(c)!=undefined&&(document.getElementById(c).checked=!0)}}recalc_onclick("")}function SetWidgetValue(obj){var widgetType=obj.attr("data-widgettype"),elemId=obj.attr("id");if(widgetType=="datepicker")TriggerOnchange(elemId);else{var strExe="$('#"+widgetType+"_"+elemId+"')."+widgetType+"('value',"+obj.val()+");";widgetType=="slider"&&(strExe=strExe+"$('#"+widgetType+"_"+elemId+"').find('a').attr('title', 'slider value='+$('#"+widgetType+"_"+elemId+"')."+widgetType+"('option','value'));"),eval(strExe)}}function ClearCookie(){var a;if(document.cookie&&document.cookie!=""){var b=document.cookie.split(";");for(var c=0;c<b.length;c++)a=b[c].split("="),a[0]=a[0].replace(/^ /,""),$.cookie(a[0],null)}}function FillValues(){var a,b=1;$("input:text, textarea").each(function(){b<151&&(a=$(this),a.val(b),b++)})}var childElement,pageName="",elementId,elementVal
